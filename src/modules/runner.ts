...
async function runApps(instances: { app: IService, event?: any }[]) {
  try {
    ...
    const neededConnections = instances.reduce((acc, instance) => (instance.app.NEED_CONNECTIONS ? acc.concat(instance.app.NEED_CONNECTIONS) : acc), []);
    await Connections.open(neededConnections);

    await Promise.all(instances.map((instance: { app: IService, event?: any }) => instance.app.start(instance.event)));
