...
export default function (sequelize: Sequelize): ITokenInstance {
  ...
  const Token = sequelize.define<ITokenAttribute>('Token', {
    id: {
      type: DataTypes.UUID,
      primaryKey: true,
      defaultValue: () => uuidv4()
  ...
  Token.findByTypeAndValueWithUser = (type, value, tOpts) => {
  ...
  Token.createToken = async (userId: string, value: string, type, requestInfo, extraValue = null, tOpts): Promise<ITokenAttribute> => {
    ...
    const rawToken: any = {
      ...
      userAgentInfo: requestInfo.userAgentInfo || {},
  ...
  Token.createForLogin = async (userId, requestInfo, tOpts): Promise<ITokenAttribute> => {
    ...
    return Token.createToken(userId, token, IEnumTokenType.AUTH, requestInfo, null, tOpts);
  };
  ...
  Token.createFor2FAEmail = async (user, requestInfo, tOpts) => {
    ...
    const token = await Token.createToken(
      user.id,
      encryptedPin,
      IEnumTokenType.TWO_FACTOR_EMAIL_LOGIN,
      requestInfo,
      null,
      tOpts
    );
  ...
  Token.createForResetPassword = async (user, requestInfo, tOpts) => {
    ...
    return Token.createToken(user.id, secretKey.base32, IEnumTokenType.RESET_PASSWORD, requestInfo, null, tOpts);
  };

  Token.createFor2Fa = async function (user, requestInfo, tOpts) {
    ...
    const token = await Token.createToken(
      user.id,
      encryptedSecretKey,
      IEnumTokenType.TWO_FACTOR_LOGIN,
      requestInfo,
      null,
      tOpts
    );
  ...
  Token.createForChangeEmail = async function (user, extraValue, requestInfo, tOpts) {
    await user.removeTokensByType(IEnumTokenType.CHANGE_EMAIL, tOpts);

    const secretKey = speakeasy.generateSecret({length: 20});

    return Token.createToken(user.id, secretKey.base32, IEnumTokenType.CHANGE_EMAIL, requestInfo, extraValue, tOpts);
  };

  Token.verify2FA = async function (user, twoFaCode, requestInfo, tOpts) {
    ...
    if (user.twoFactor) {
      ...
      assertExposable(
        moment(twoFaToken.updatedAt).isAfter(moment().subtract({minutes: config.SERVICES.API.MAIL_PIN_EXPIRATION})),
        'token_expired'
      );
    }
