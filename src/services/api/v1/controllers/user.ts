...
import {assertExposable, throwExposable} from '../../../../helpers/errors';
...
import {IAskTwoFactorRes, IEnumLoginType, ILoginRes, ILoginTypeRes, IUserRes} from "../../../../types/routers/res/user";
import moment from "moment";
import logger from "../../../../modules/logger";
import AuthMiddleware from "../middlewares/auth";
import Notification from "../../../../modules/notification";

const llo = logger.logMeta.bind(null, {service: 'api:controller:user'});
...
const UserController = {
  ...
  createUser: async (rawUser: ISignup, opts: IRequestInfo): Promise<ILoginTypeRes> => {
    ...
    const user = await DB.executeTxFn(async (tOpts: ITxOpts) => {
  ...
  askLogin: async ({email, password}: IAskLogin, opts: IRequestInfo): Promise<ILoginTypeRes> => {
    ...
    const {user, pinCode} = await DB.executeTxFn(async (tOpts: ITxOpts) => {
  ...
  login: async ({email, password, twoFaCode}: ILogin, opts: IRequestInfo): Promise<ILoginRes> => {
    ...
    const {user, jwt} = await DB.executeTxFn(async (tOpts: ITxOpts) => {
  ...
  askResetPassword: async (email: string, opts: IRequestInfo): Promise<boolean> => {
    ...
    const token = await DB.executeTxFn(async (tOpts: ITxOpts) => {
      await UserController._checkFailedLoginAttempts(user, tOpts);
  ...
  resetPassword: async ({token, newPassword}: IResetPassword): Promise<boolean> => {
    ...
    const tokenDb = await DB.executeTxFn(async (tOpts: ITxOpts) => {
      ...
      assertExposable(moment(tokenDb.createdAt).isAfter(moment().subtract({hours: config.SERVICES.API.MAIL_RESET_PASSWORD_EXPIRATION})), 'token_expired');
  ...
  changeEmail: async (token: string): Promise<boolean> => {
    ...
    const {tokenDb, newEmail} = await DB.executeTxFn(async (tOpts: ITxOpts) => {
  ...
  async changePassword(user: IUserAttribute, {oldPassword, newPassword}: IChangePassword): Promise<boolean> {
    ...
    await DB.executeTxFn(async (tOpts: ITxOpts) => {
  ...
  async askTwoFactor(user: IUserAttribute, requestInfo: IRequestInfo): Promise<IAskTwoFactorRes> {
    ...
    const secret = await DB.executeTxFn(async (tOpts: ITxOpts) => {
  ...
  _checkFailedLoginAttempts: async (user: IUserAttribute, tOpts = {}): Promise<boolean> => {
    ...
    if (!user.isActive && newFailedLoginCount > config.SERVICES.API.LOGIN_RETRY_ATTEMPTS + 10) {
    ...
    await user.update({
      isActive: newFailedLoginCount >= config.SERVICES.API.LOGIN_RETRY_ATTEMPTS ? false : user.isActive,
      countLoginFailed: newFailedLoginCount,
    }, tOpts);

    if (newFailedLoginCount >= config.SERVICES.API.LOGIN_RETRY_ATTEMPTS) {
